...
### AD-14 — One person record; erasure clears that record and nothing else
...
- **Rule:** No record copies a person's name or number; every record references one person record, whose key is opaque (AD-28) — otherwise Frappe's `owner` and `modified_by` columns reproduce the identifier on every row and erasure becomes impossible. Erasure replaces the identifying fields there with a stable non-identifying token, and every referring record shows the token while keeping its content — a Review keeps its text with the author de-identified, an Agreement keeps its terms, the counterparty loses nothing. NFR 5.5's precedence is implemented here and never re-derived: consent-based data is erased; engagement records and legally-required records are pseudonymised and retained. **A Guest is a person record scoped to one Wedding**, carrying AD-32's retention outcome `erase` for the reason *purpose-limited* — so there is one erasure mechanism, not two. It is **never deduplicated across Weddings and never joined to any other person record**: FR-12 confines a Guest's details to composing that Wedding's invitations, and a shared row would make audience-building structural and let a guest purge clear a live Vendor's fields. Its trigger is the purpose ending rather than a request: 30 days after the Wedding concludes, immediately on abandonment, immediately when the Creator dismisses a guest-form submission. The token is **stable per person**, so FR-63's repeat-infringer register survives erasure.
...
erDiagram
    ...
    WEDDING ||--o{ GUEST : optional
    PERSON ||--o| GUEST : "person record, scoped to one Wedding"
    ...
    PERSON ||--o{ AGREEMENT : party
    PERSON ||--o{ REVIEW : author
    PERSON ||--o{ CONSENT : "basis per purpose"
    PERSON ||--o{ BREACH_INCIDENT : "affected, enumerated"
...
`PERSON` is the single identity record AD-14 turns on. `SERVICE_DETAIL` stands for the per-Service detail DocType of AD-6.
